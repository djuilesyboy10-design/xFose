...
void FOSE_Initialize(void)
{
	...
	__try {
		...
		_MESSAGE("xFOSE runtime: initialize (version = %d.%d.%d %08X %08X%08X)",
		...
		// Initialize Event Manager before CommandTable::Init() which loads plugins
...
{
	_MESSAGE("xFOSE: deinitialize");
	
	// Shutdown Event Manager
	EventManager::Shutdown();
	_MESSAGE("Event Manager shutdown");
}
