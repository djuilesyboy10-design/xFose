...
// 8 - Original String class for backward compatibility
// NOTE: Memory layout (m_data, m_dataLen, m_bufLen) must NOT change - mirrors game's internal structure
class String
{
public:
	String();
	...
	// Modernization: Copy constructor and assignment operators
	String(const String& other);
	String& operator=(const String& other);
	String& operator=(const char* src);
	String& operator=(const std::string& str);

	// Modernization: Modern helper methods
	const char*	c_str() const { return m_data ? m_data : ""; }
	UInt16		length() const { return m_dataLen; }
	bool		empty() const { return m_dataLen == 0 || m_data == NULL; }
	...
	// Modernization: Append operator
	String& operator+=(const char* toAppend);
	String& operator+=(const String& other);
};
...
public:
	...
	public:
		...
		// Modernization: operator!= for range-based for loop support
		bool operator!=(const Iterator& other) const { return m_cur != other.m_cur; }
	};
	
	const Iterator Begin() const { return Iterator(Head()); }

	// Modernization: begin() and end() for range-based for loops
	...
	// Modernization: STL-like aliases
	...
	// Modernization: STL-like convenience wrappers
	Item* front() const { return GetNthItem(0); }
	Item* back() const { return GetLastItem(); }

	// Modernization: operator[] for indexed access
	Item* operator[](SInt32 index) const { return GetNthItem(index); }
	...
	// Modernization: STL-like convenience wrappers
	void clear() const { RemoveAll(); }
	SInt32 push_back(Item* item) { return AddAt(item, eListEnd); }
	SInt32 push_front(Item* item) { return AddAt(item, 0); }
...
public:
	...
	// Modernization: STL-like methods
	UInt32 size() const { return m_numItems; }
	bool empty() const { return m_numItems == 0; }
	UInt32 bucket_count() const { return m_numBuckets; }
};
...
struct NiTArray
{
	...
	T at(UInt32 idx) const { return (*this)[idx]; }
	T front() const { return (length > 0) ? data[0] : NULL; }
	T back() const { return (length > 0) ? data[length - 1] : NULL; }
};
...
struct BSSimpleArray
{
	...
	T at(UInt32 idx) const { return (*this)[idx]; }
	T front() const { return (size > 0) ? data[0] : NULL; }
	T back() const { return (size > 0) ? data[size - 1] : NULL; }
	UInt32 capacity() const { return alloc; }
};
...
public:
	...
	UInt32 size() const { return m_numItems; }
	bool empty() const { return m_numItems == 0; }
	Iterator begin() { return Iterator(this, NULL, 0); }
	Iterator end() { return Iterator(this, NULL, m_numBuckets); }
};
